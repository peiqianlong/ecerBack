        ...
        <div class="pro">
          <div class="memu">
            <div class="proitem" style="width:100%;">
              ...
              <div class="mar" style="float:right;12px">
                <savebtn tit="添加产品" @Fun="addProduct"></savebtn>
              </div>
            </div>
          </div>
          <div class="table" style=" padding-bottom: 0px;">
            <transition name="el-zoom-in-top">
              <div class="operation" v-if="menuShow">
                <div @click="ProductUpDown('1')">上架</div>
                <div @click="ProductUpDown('0')">下架</div>
            ...
            >
              ...
              <el-table-column label="图片" width="148">
                <template slot-scope="scope">
                  <div class="img" @click="addProduct(scope.row)">
                    <img
                      class="jqimg"
                      v-if="scope.row.check_product_img_list"
                      :src="scope.row.check_product_img_list[0].media_url_http+'_1000x750.'+scope.row.check_product_img_list[0].media_suffix"
                      alt
                    />
                    <img
                      v-else
              ...
              <el-table-column label="名称">
                <template slot-scope="scope">
                  <div style="display:flex;padding-right: 15px;">
                    <div class="name" @click="addProduct(scope.row)">{{ scope.row.product_name }}</div>
              ...
              <el-table-column label="状态" width="102">
                <template slot-scope="scope">
                  <div v-if="scope.row.product_status == 1" class="pruductUp"></div>
                  <div v-if="scope.row.product_status == 2" class="pruductDown"></div>
                  <div v-if="scope.row.product_status == 0" class="pruductDown"></div>
                </template>
              </el-table-column>
...
export default {
  ...
  components: {
    addproduct: resolve =>
      require(["../../components/Product/add.vue"], resolve),
    savebtn: resolve =>
  ...
  methods: {
    ...
    getDataInfo() {
      ProductList().then(res => {
        if (res.status === 200) {
          this.tableData3 = res.data.data.product_list;
